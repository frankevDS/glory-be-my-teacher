...
export function buildQuizPrompt({
  ...
  const exclusionLine =
    excludeQuestions?.length > 0
      ? `\nThe student has already seen these ${excludeQuestions.length} questions on this exact topic in earlier rounds — write 10 GENUINELY DIFFERENT ones, not these:\n${excludeQuestions.map((q, i) => `${i + 1}. ${q}`).join("\n")}\nIf you're struggling to find fresh angles because the topic is narrow, don't just reword the same question — change what's being tested (a different sub-concept, a different example scenario, a different number/value in a calculation, a different real-world context) so it's a genuinely new question, not a disguised repeat.\n`
      : "";
...
export function buildPuzzlePrompt({
  ...
  const exclusionLine =
    excludeWords?.length || excludeSentences?.length
      ? `\nThe student has already seen these in previous rounds — you MUST NOT reuse any of them, and must pick genuinely different words/sentences this time (there is more than one reasonable vocabulary word and example sentence for almost any topic; dig for the next-best ones instead of repeating):\nAlready-used words: ${(excludeWords || []).join(", ") || "(none)"}\nAlready-used sentences: ${(excludeSentences || []).join(" | ") || "(none)"}\nIf the topic is narrow and you're running low on genuinely distinct options, don't quietly repeat one — instead approach an already-covered idea from a different angle (a different example, a different sentence structure, a different real-life context) so it still reads as new.\n`
      : "";
